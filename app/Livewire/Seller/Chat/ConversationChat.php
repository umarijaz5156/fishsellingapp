...
class ConversationChat extends Component
{
    ...
    public function sendMessage()
    {
        ...
        // sendEmail
        $user = User::findOrFail($conversation->buyer_id);
        $senderName = User::findOrFail(auth()->user()->id);   

        $customerEmail = $user->email;
        $subject = "New Message from Seller";
        $heading = "Message from Seller";

        $body = "Hello ". $user->name .",<br><br>
        You have received a new message from  <strong>{$senderName->name}</strong> (seller). Below is the message you received:<br><br>
    ...
    {
        return view('livewire.seller.chat.conversation-chat');
    }
}
