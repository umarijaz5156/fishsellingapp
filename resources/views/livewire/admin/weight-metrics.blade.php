<div class="mx-8">
                    ...
                    <x-admin.table>
                        <x-admin.table.thead>
                            <tr>
                                ...
                                <th
                                class="px-2 py-3 font-bold text-left uppercase align-middle bg-transparent border-b border-gray-200 shadow-none text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70">
                                <button wire:click="sortBy('abbreviation')" type="button" class="flex">
                                    Abbreviation
                                    <x-sort-icon field="abbreviation" :sortField="$sortField" :sortAsc="$sortAsc" />
                                </button>
                            </th>
                        ...
                        <tbody>
                            ...
                            @forelse ($metrics as $metric)
                                <tr>
                                    ...
                                    <x-admin.table.cell>
                                        <button type="button" wire:click="editMetic({{ $metric->id }})"
                                            ...
                                            Edit</button>
                                        <button type="button" wire:click="deleteMetric({{ $metric->id }})"
    ...
    <x-modals.delete-alert message="You are going to delete Metric" />

    @if ($addMetricsModal)
        <x-modals.modal wire:model.live="addMetricsModal" maxWidth="2xl">
            @slot('headerTitle')
               @if ($metricEditId)
               {{ 'Edit Weight Metric' }}
               @else
               {{ 'Add new Weight Metric' }}
               @endif 
            @endslot

            @slot('content')
                <form class="my-5 space-y-6" wire:submit.prevent="createWeightMertic">

                    <div class=" mb-4">
                        <div class="">
                            <x-admin.form.label for="weightName">Name</x-admin.form.label>
                            <x-admin.form.input wire:model="name" maxlength="30" type="text" id="weightName" placeholder="Enter Name (weight metric Kilogram, Tons, Gram)" />
                            <x-input-error for="name" />
                        </div>
                    </div>
                    <div class=" mb-4">
                        <div class="">
                            <x-admin.form.label for="weightName">Abbreviation</x-admin.form.label>
                            <x-admin.form.input wire:model="abbreviation" maxlength="30" type="text" id="abbreviation" placeholder="Enter abbreviation" />
                            <x-input-error for="abbreviation" />
                        </div>
                    </div>
